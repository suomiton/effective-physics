...
function initApp(): void {
  ...
  const originalCanvas = document.getElementById('simulationCanvas') as HTMLCanvasElement;
  if (!originalCanvas) {
    console.error('Canvas element not found. Make sure there is a canvas with id "simulationCanvas".');
    return;
  }
  ...
  const block = PhysicsUtils.createBlock(engine.world, canvas.width / 2, canvas.height / 2);
  ...
  let currentRenderer = rendererSelect ? rendererSelect.value : Constants.RENDERER.WEBGL;
  if (!currentRenderer) {
...
function initializeRenderer(
  ...
  if (rendererType === Constants.RENDERER.CANVAS_2D) {
    ...
    try {
      ...
      initializeRenderer(Constants.RENDERER.CANVAS_2D, engine, canvas, container);
    }
  } else {
