...
export function createWebGLRenderer(
  ...
  const config: RendererWebGLConfig = {
    ...
    defaultObjectColor: options.defaultObjectColor || Constants.COLORS.DEFAULT_OBJECT,
    hasShadows: options.hasShadows !== undefined ? options.hasShadows : true,
    usesOrthographicCamera: options.usesOrthographicCamera !== undefined ? options.usesOrthographicCamera : true
  };
  ...
  const rendererInstance: RendererWebGLInstance = {
     ...
     */
    getMouseCoordinates: function(clientX: number, clientY: number): { x: number, y: number } {
