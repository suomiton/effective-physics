...
export function MatterThree(options: {
  ...
  const instance: MatterThreeInstance = {
    ...
    createBodyMesh: function(body: Matter.Body): THREE.Mesh {
      ...
      const material = new THREE.MeshLambertMaterial({
        color: body.render && body.render.fillStyle 
      ...
      if (body.circleRadius) {
        ...
        if (body.vertices && body.vertices.length > 0) {
          const firstVertex = Matter.Vector.sub(body.vertices[0], body.position);
  ...
  (function initialize(): void {
    ...
    instance.renderer.setClearColor(
      new THREE.Color(instance.background), 
      1
    );
